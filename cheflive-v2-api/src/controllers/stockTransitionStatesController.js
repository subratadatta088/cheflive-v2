...
const { StockTransitionStateService } = require('../services/stockTransitionStateService')

function isSuperAdmin(req) {
  return Array.isArray(req.user?.roles) && req.user.roles.includes('superadmin')
}

class StockTransitionStatesController {
  list = async (req, res) => {
    const parsed = StockTransitionStateListQuerySchema.safeParse(req.query)
    ...
    const service = new StockTransitionStateService({ models: req.models, user: req.user })
    const items = await service.list(query)
    return res.json({ page: query.page, limit: query.limit, items })
  }

  getById = async (req, res) => {
    const idParsed = StockTransitionStateIdSchema.safeParse(Number(req.params.id))
    if (!idParsed.success) return res.status(400).json({ error: 'Invalid id' })

    const service = new StockTransitionStateService({ models: req.models, user: req.user })
    const item = await service.getById(idParsed.data)
    if (!item) return res.status(404).json({ error: 'Not found' })
    return res.json({ stock_transition_state: item })
  }
}

module.exports = { StockTransitionStatesController }
